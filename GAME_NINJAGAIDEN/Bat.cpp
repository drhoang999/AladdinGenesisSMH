...
Bat::Bat(int direction, float X, float Y, int status)
{
	x = X;
	y = Y;
...
void Bat::Update(DWORD dt, float xAladdin, float yAladdin, int dAladdin, vector<LPGAMEOBJECT>* coObjects)
{
	...
	else // bay ngang
	{
		...
		if (abs(x - xStart) > delta)
		{
			...
			{
				backupNinjaY = yAladdin; //phòng trường hợp bird đang bay xuống thì ninja lại nhảy nên yBat ko thể dừng
				follow = true;
			}
		}
	}
...
void Bat::RenderActiveBoundingBox(Camera* camera)
{
	float l, t, r, b;
