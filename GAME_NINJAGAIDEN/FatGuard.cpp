...
FatGuard::FatGuard(int direction, float X, float Y, int status)
{
	x = X;
	y = Y;
	...
	texture = TextureManager::GetInstance()->GetTexture(eType::FAT_GUARD);
	sprite = new CSprite(texture, 100);


	type = eType::FAT_GUARD;
...
void FatGuard::Update(DWORD dt, float xAladdin, float yAladdin, int dAladdin, vector<LPGAMEOBJECT>* coObjects)
{
	...
	else // bay ngang
	{
		...
		if (abs(x - xStart) > delta)
		{
			...
			{
				backupNinjaY = yAladdin; //phòng trường hợp bird đang bay xuống thì ninja lại nhảy nên yFatGuard ko thể dừng
				follow = true;
			}
		}
	}
...
void FatGuard::Render(Camera* camera)
{
...
void FatGuard::GetActiveBoundingBox(float& left, float& top, float& right, float& bottom, int id)
{
...
void FatGuard::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = x + 15;
	top = y - 1;
	right = x + 40;
...
void FatGuard::RenderActiveBoundingBox(Camera* camera)
{
	float l, t, r, b;
